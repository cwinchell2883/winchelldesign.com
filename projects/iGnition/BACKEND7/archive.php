...
{
	$where = "WHERE sp_news.section = $_REQUEST[section]";
}
...
if ($search == 1) {
	...
	$result = $db->Execute("SELECT id,title,author,date,section FROM sp_news WHERE date LIKE '".$year.".".$month.".%' $where ORDER BY date DESC");
	} else {
	$result = $db->Execute("SELECT id,title,author,date,section FROM sp_news ORDER BY date DESC LIMIT 0,50");
	}

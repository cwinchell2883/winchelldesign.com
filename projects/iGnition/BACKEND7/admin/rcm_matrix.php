...
include "global.php";
include "../sources/admin/matrix.class.php";
...
function delete_resource() {
   global $db;
   $db->Execute("DELETE FROM `sp_matrix` WHERE `id` = '$_REQUEST[did]'");
   viewmatrix();
   }

function create_confirm() {
   global $db;
   $rs = $db->Execute("SELECT * FROM `sp_matrix` WHERE `id` = '-1'");
   $record = array(
      'cid' => $_REQUEST['id'],
      'ctype' => $_REQUEST['type'],
...
function search2() {
   ...
   switch ($_REQUEST['type']) {
      case 'null':
         ...
         break;
      case 'companies':
         $result = $db->Execute("SELECT * FROM `sp_companies` WHERE `title` LIKE '$_REQUEST[keywords]%' ORDER BY `title`");
         while ($row = $result->FetchNextObject()) {
            echo "<tr><td><a href=\"rcm_matrix.php?type=company&do=viewmatrix&id=$row->ID\">" . stripslashes($row->TITLE) . " (View Matrix)</a></td></tr>";
         }
         break;
      case 'games':
         $result = $db->Execute("SELECT * FROM `sp_games` WHERE `title` LIKE '$_REQUEST[keywords]%' ORDER BY `title`");
         while ($row = $result->FetchNextObject()) {
            echo "<tr><td><a href=\"rcm_matrix.php?type=games&do=viewmatrix&id=$row->ID\">" . stripslashes($row->TITLE) . " (View Matrix)</a></td></tr>";
         }
         break;
      case 'links':
         $result = $db->Execute("SELECT * FROM `sp_links` WHERE `title` LIKE '$_REQUEST[keywords]%' ORDER BY `title`");
         while ($row = $result->FetchNextObject()) {
            echo "<tr><td><a href=\"rcm_matrix.php?type=links&do=viewmatrix&id=$row->ID\">" . stripslashes($row->TITLE) . " (View Matrix)</a></td></tr>";
         }
         break;
      case 'news':
         $result = $db->Execute("SELECT * FROM `sp_news` WHERE `title` LIKE '$_REQUEST[keywords]%' ORDER BY `title`");
         while ($row = $result->FetchNextObject()) {
            echo "<tr><td><a href=\"rcm_matrix.php?type=news&do=viewmatrix&id=$row->ID\">" . stripslashes($row->TITLE) . " (View Matrix)</a></td></tr>";
         }
         break;
      case 'previews':
         $result = $db->Execute("SELECT * FROM `sp_previews` WHERE `title` LIKE '$_REQUEST[keywords]%' ORDER BY `title`");
         while ($row = $result->FetchNextObject()) {
            echo "<tr><td><a href=\"rcm_matrix.php?type=previews&do=viewmatrix&id=$row->ID\">" . stripslashes($row->TITLE) . " (View Matrix)</a></td></tr>";
         }
         break;
      case 'reviews':
         $result = $db->Execute("SELECT * FROM `sp_reviews` WHERE `title` LIKE '$_REQUEST[keywords]%' ORDER BY `title`");
         while ($row = $result->FetchNextObject()) {
            echo "<tr><td><a href=\"rcm_matrix.php?type=reviews&do=viewmatrix&id=$row->ID\">" . stripslashes($row->TITLE) . " (View Matrix)</a></td></tr>";
         }
         break;
      case 'pages':
         $result = $db->Execute("SELECT * FROM `sp_pages` WHERE `title` LIKE '$_REQUEST[keywords]%' ORDER BY `title`");
         while ($row = $result->FetchNextObject()) {

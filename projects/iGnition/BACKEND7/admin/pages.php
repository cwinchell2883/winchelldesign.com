...
if (empty($_REQUEST['do'])) {
   ...
   $result = $db->Execute("SELECT id,title FROM `sp_pages`");
...
if ($_REQUEST['do'] == 'Edit Page') {
   $menuitem = $db->Execute("SELECT * FROM `sp_pages` WHERE `id` = '$_REQUEST[id]'");
   do_form_header('pages.php');
...
if ($_REQUEST['do'] == 'add_page_confirm') {
   ...
   $rs = $db->Execute("SELECT * FROM `sp_pages` WHERE `id` = '-1'");
   $record = array(
...
if ($_REQUEST['do'] == 'edit_page_confirm') {
   ...
   $rs = $db->Execute("SELECT * FROM `sp_pages` WHERE `id` = '$_REQUEST[id]'");
   $record = array(
      'title' => $_REQUEST['title'],
      'content' => $_REQUEST['content']
      );
...
if ($_REQUEST['do'] == 'Delete Page') {
   ...
   $db->Execute("DELETE FROM `sp_pages` WHERE `id` = '$_REQUEST[id]'");

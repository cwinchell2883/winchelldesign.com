...
if (empty($_REQUEST['do']))
{
   ...
   $result = $db->Execute("SELECT * FROM `sp_menu_items` WHERE `location` = 'left' ORDER BY `ordering`");
   ...
   $result = $db->Execute("SELECT * FROM `sp_menu_items` WHERE `location` = 'right' ORDER BY `ordering`");
   while ($row = $result->FetchNextObject()) {
      $bgcolor = ($bgcolor == "#ECECFF" ? "#FFFFFF" : "#ECECFF");
      echo '<tr>
   ...
   $result = $db->Execute("SELECT * FROM `sp_menu_items` WHERE `location` = 'top' ORDER BY `ordering`");
   while ($row = $result->FetchNextObject()) {
      $bgcolor = ($bgcolor == "#ECECFF" ? "#FFFFFF" : "#ECECFF");
      echo '<tr>
            <td bgcolor="'.$bgcolor.'"><input type="radio" name="id" value="'.$row->ID.'"> '.$row->TITLE.'</td>
   ...
   $result = $db->Execute("SELECT * FROM `sp_menu_items` WHERE `location` = 'bottom' ORDER BY `ordering`");
   while ($row = $result->FetchNextObject()) {
      $bgcolor = ($bgcolor == "#ECECFF" ? "#FFFFFF" : "#ECECFF");
      echo '<tr>
            <td bgcolor="'.$bgcolor.'"><input type="radio" name="id" value="'.$row->ID.'"> '.$row->TITLE.'</td>
...
if ($_REQUEST['do'] == 'add_link_step2') {
   ...
   if ($_REQUEST['type'] == 'preview_section') {
      ...
      do_form_header('menu_manager.php');
      do_table_header('Add Menu Item');
      $result = $db->Execute("SELECT id,title FROM `sp_previews_sections` ORDER BY `title`");
      while ($row = $result->FetchNextObject()) {
   ...
   if ($_REQUEST['type'] == 'news_section') {
      ...
      do_form_header('menu_manager.php');
      do_table_header('Add Menu Item');
      $result = $db->Execute("SELECT id,title FROM `sp_newss_sections` ORDER BY `title`");
      while ($row = $result->FetchNextObject()) {
   ...
   if ($_REQUEST['type'] == 'link_section') {
      ...
      do_form_header('menu_manager.php');
      do_table_header('Add Menu Item');
      $result = $db->Execute("SELECT id,title FROM `sp_links_sections` ORDER BY `title`");
      while ($row = $result->FetchNextObject()) {
   ...
   if ($_REQUEST['type'] == 'module') {
      ...
      echo '<tr><td class="formlabel" valign="top" align="right"><b>Modules</b></td>
                <td class="formlabel">
                  ...
                  <option value="games.php">Games List</option>
                  <option value="previews.php">Previews</option>
                  <option value="reviews.php">Reviews</option>
				  <option value="rss_games.php">RSS Feed - Latest Games</option>
...
if ($_REQUEST['do'] == 'Edit Link') {
   $menuitem = $db->Execute("SELECT * FROM `sp_menu_items` WHERE `id` = '$_REQUEST[id]'");
   do_form_header('menu_manager.php');
...
if ($_REQUEST['do'] == 'add_link_confirm') {
   ...
   $rs = $db->Execute("SELECT * FROM `sp_menu_items` WHERE `id` = '-1'");
   $record = array(
...
if ($_REQUEST['do'] == 'edit_link_confirm') {
   ...
   $rs = $db->Execute("SELECT * FROM `sp_menu_items` WHERE `id` = '$_REQUEST[id]'");
   $record = array(
      'title' => $_REQUEST['title'],
      'url' => $_REQUEST['url'],
      'target' => $_REQUEST['target'],
...
if ($_REQUEST['do'] == 'Delete Link') {
   ...
   $db->Execute("DELETE FROM `sp_menu_items` WHERE `id` = '$_REQUEST[id]'");

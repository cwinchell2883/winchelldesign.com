...
if (!isset($_REQUEST['do'])) {
	...
	$sections = $db->Execute("SELECT * FROM `sp_links_sections` ORDER BY `id`");
	$spSections = array();
	...
	if (isset($_REQUEST['s'])) {
		$latestLink = $db->Execute("SELECT id,title,section FROM `sp_links` WHERE `section` = '$_REQUEST[s]' ORDER BY `id` DESC");
	} else {
		$latestLink = $db->Execute("SELECT id,title,section FROM `sp_links` ORDER BY `id` DESC LIMIT 20");
	}
...
if ($_REQUEST['do'] == 'add_link') {
	...
	$sections = $db->Execute("SELECT id,title FROM `sp_links_sections` ORDER BY `title`");
...
if ($_REQUEST['do'] == 'add_link_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_links` WHERE `id` = '-1'");
	$record = array(
...
if ($_REQUEST['do'] == 'edit_link_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_links` WHERE `id` = '{$_REQUEST['id']}'");
	$record = array(
		'title' => $_REQUEST['title'],
		'url' => $_REQUEST['url'],
		'section' => $_REQUEST['section']
...
if ($_REQUEST['do'] == 'Edit Link') {
	...
	$link = $db->Execute("SELECT * FROM `sp_links` WHERE `id` = '" . $_REQUEST['id'] . "'");
	$sections = FetchSections('sp_links_sections');
...
if ($_REQUEST['do'] == 'Edit Section') {
	$section = $db->Execute("SELECT * FROM `sp_links_sections` WHERE `id` = '$_REQUEST[id]'");
	do_form_header('links.php');
...
if ($_REQUEST['do'] == 'edit_section_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_links_sections` WHERE `id` = '$_REQUEST[id]'");
	$record = array(
		'title' => $_REQUEST['title']
		);
	$sql = $db->GetUpdateSQL($rs, $record);
...
if ($_REQUEST['do'] == 'add_section_confirm') {
	...
	$db->Execute("INSERT INTO `sp_links_sections` (title) VALUES ('$_REQUEST[title]');");
...
if ($_REQUEST['do'] == 'Delete Section') {
	...
	$db->Execute("DELETE FROM `sp_links_sections` WHERE `id` = '$_REQUEST[id]'");

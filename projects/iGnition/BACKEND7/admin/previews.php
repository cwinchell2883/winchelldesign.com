...
include "global.php";
include("../sources/admin/previews.class.php");
$cp->header();
...
if (empty($_REQUEST['do'])) {
	...
	$sections = $db->Execute("SELECT * FROM `sp_previews_sections` ORDER BY `id`");
	$spSections = array();
	...
	if (isset($_REQUEST['s'])) {
		$latestPreview = $db->Execute("SELECT id,title,section FROM `sp_previews` WHERE `section` = '$_REQUEST[s]' ORDER BY `id` DESC");
	} else {
		$latestPreview = $db->Execute("SELECT id,title,section FROM `sp_previews` ORDER BY `id` DESC LIMIT 20");
	}
...
if ($_REQUEST['do'] == 'Edit Preview') {
	$news = $db->Execute("SELECT * FROM `sp_previews` WHERE `id` = '$_REQUEST[id]';");
	do_form_header('previews.php');
...
if ($_REQUEST['do'] == 'edit_news_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_previews` WHERE `id` = '$_REQUEST[id]'");
	$record = array(
...
if ($_REQUEST['do'] == 'Edit Section') {
	$section = $db->Execute("SELECT * FROM `sp_previews_sections` WHERE `id` = '$_REQUEST[id]'");
	do_form_header('previews.php');
...
if ($_REQUEST['do'] == 'edit_section_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_previews_sections` WHERE `id` = '$_REQUEST[id]'");
	$record = array(
		'title' => $_REQUEST['title']
		);
	$sql = $db->GetUpdateSQL($rs, $record);
...
if ($_REQUEST['do'] == 'add_section_confirm') {
	...
	$db->Execute("INSERT INTO `sp_previews_sections` (title) VALUES ('$_REQUEST[title]');");
...
if ($_REQUEST['do'] == 'add_news_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_previews` WHERE `id` = '-1'");
	$record = array(
		'title' => $_REQUEST['title'],
		'section' => $_REQUEST['section'],
		'intro' => $_REQUEST['intro'],
...
if ($_REQUEST['do'] == 'Delete Section') {
	...
	$db->Execute("DELETE FROM `sp_previews_sections` WHERE `id` = '$_REQUEST[id]'");
...
if ($_REQUEST['do'] == 'Delete Preview') {
	...
	$db->Execute("DELETE FROM `sp_previews` WHERE `id` = '$_REQUEST[id]'");

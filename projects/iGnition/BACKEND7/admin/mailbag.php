...
if (empty($_REQUEST['do'])) {
	...
	$latestLetter = $db->Execute("SELECT id,title FROM `sp_mailbag` ORDER BY `id` DESC LIMIT 20");
...
if ($_REQUEST['do'] == 'Edit Letter') {
	$mailbag = $db->Execute("SELECT * FROM `sp_mailbag` WHERE `id` = '$_REQUEST[id]';");
	do_form_header('mailbag.php');
...
if ($_REQUEST['do'] == 'edit_mailbag_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_mailbag` WHERE `id` = '$_REQUEST[id]'");
	$record = array(
...
if ($_REQUEST['do'] == 'add_mailbag_confirm')
{
	...
	$db->AutoExecute('sp_mailbag',$record,'INSERT');
	SPMessage("Success | Letter has been added.","mailbag.php");
}

if ($_REQUEST['do'] == 'Delete Letter')
{
	$db->Execute("DELETE FROM `sp_mailbag` WHERE `id` = '$_REQUEST[id]'");

...
require_once( '../sources/admin/news.class.php' );
...
if (empty($_REQUEST['do']))
{
	...
	$sections = $db->Execute("SELECT * FROM `sp_news_sections` ORDER BY `id`");
	$spSections = array();
	...
	if (isset($_REQUEST['s'])) {
		$latestNews = $db->Execute("SELECT id,title,section FROM `sp_news` WHERE `section` = '$_REQUEST[s]' ORDER BY `id` DESC");
	} else {
		$latestNews = $db->Execute("SELECT id,title,section FROM `sp_news` ORDER BY `id` DESC LIMIT 20");
	}
...
if ($_REQUEST['do'] == 'Edit News') {
	$news = $db->Execute("SELECT * FROM `sp_news` WHERE `id` = '$_REQUEST[id]';");
	$sections = FetchSections('sp_news_sections');
	do_form_header('news.php');
...
if ($_REQUEST['do'] == 'edit_news_confirm') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_news` WHERE `id` = '$_REQUEST[id]'");
	$record = array(
...
{
	$section = $db->Execute("SELECT * FROM `sp_news_sections` WHERE `id` = '$_REQUEST[id]'");
	do_form_header('news.php');
...
{
	$rs = $db->Execute("SELECT * FROM `sp_news_sections` WHERE `id` = '$_REQUEST[id]'");
	$record = array(
		'title' => $_REQUEST['title']
		);
	$sql = $db->GetUpdateSQL($rs, $record);
...
{
	$db->Execute("INSERT INTO `sp_news_sections` (title) VALUES ('$_REQUEST[title]');");
...
if ($_REQUEST['do'] == 'manage_sections')
{
	...
	$result = $db->Execute("SELECT id,title FROM `sp_news_sections` ORDER BY `title`");
...
{
	$db->Execute("DELETE FROM sp_news_sections WHERE id = $_REQUEST[id]");
...
{
	$db->Execute("DELETE FROM sp_news WHERE id = $_REQUEST[id]");

...
include_once("../sources/admin/polls.class.php");
$cp->header();
$module = new Module;
...
{
	$option = $db->Execute("SELECT * FROM sp_polls_options WHERE id = '$_REQUEST[id]'") or die($db->ErrorMsg());
	do_form_header( 'polls.php' );
...
if ($_REQUEST['do'] == 'edit_option_confirm')
{
	...
	$db->AutoExecute('sp_polls_options',$result,'UPDATE',"id = '$_REQUEST[id]'");
...
if ($_REQUEST['do'] == 'Delete Option')
{
	...
	{
		$db->Execute("DELETE FROM sp_polls_options WHERE id = '$_REQUEST[id]'");

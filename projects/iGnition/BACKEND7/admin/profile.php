...
if (!isset($_REQUEST['do'])) {
	...
	$result = $db->Execute("SELECT * FROM `sp_members` WHERE `ID` = '" . $_SESSION['pwzid'] . "'");
...
if ($_REQUEST['do'] == 'save_profile') {
	...
	$rs = $db->Execute("SELECT * FROM `sp_members` WHERE `ID` = '" . $_SESSION['pwzid'] . "'");
	$record = array(
	...
	if (isset($_REQUEST['curpass'])) {
		if (md5($_REQUEST['curpass']) == $rs->fields['PASS']) {
			...
			if ( (!empty($_REQUEST['newpass'])) && ($_REQUEST['newpass'] == $_REQUEST['newpass_confirm']) ) {
				$db->Execute("UPDATE `sp_members` SET `PASS` = '" . md5($_REQUEST[newpass]) . "' WHERE `id` = '" . $_SESSION['pwzid'] . "'");
			}
		} else {

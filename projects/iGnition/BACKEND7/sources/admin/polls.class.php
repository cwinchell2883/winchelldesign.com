...
class Module
{
	...
	function add_options_confirm()
	{
		...
		$db->Execute("INSERT INTO `sp_polls_options` (poll_id,text) VALUES $options;");
	...
	function add_poll_confirm()
	{
		...
		$db->AutoExecute('sp_polls',$record,'INSERT');
		$result = $db->Execute("SELECT id FROM sp_polls ORDER BY `id` DESC LIMIT 1");
	...
	{
		global $db;
		$db->Execute("DELETE FROM `sp_polls` WHERE `id` = '$_REQUEST[id]'");
		$db->Execute("DELETE FROM `sp_polls_options` WHERE `poll_id` = '$_REQUEST[id]'");

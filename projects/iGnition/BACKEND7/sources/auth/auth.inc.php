...
if ($_REQUEST['pwzpath']||$_REQUEST['pwzrequire']||$_REQUEST['login_not_required']) {
...
if ( (!$_SESSION["pwzlogin"]) && (!$_SESSION["pwzpassword"]) && ($login_not_required==1) && !isset($_REQUEST['login_not_required']) ) {
	...
	$userQuery = pwz_db("
		SELECT
			sp_members.ID AS ID, sp_members.PSEUDO AS PSEUDO, sp_members.PASS AS PASS,
			sp_members.PRIV AS PRIV, sp_members.EXPIRE AS EXPIRE
		FROM
			sp_members
		WHERE
			PSEUDO = '{$_SESSION["pwzlogin"]}' AND
			ACTIF = '1'", $pwzsql);
	...
	if (mysql_num_rows($userQuery) != 0) {
		...
		if ((strtolower($_SESSION["pwzlogin"]) != strtolower($userArray['PSEUDO']) && $pwzcase==0)||($_SESSION["pwzlogin"] != $userArray['PSEUDO'] && $pwzcase==1)) {
	...
	if ($userArray["PASS"] != $_SESSION["pwzpassword"]) {
		// mot de passe FAUX

...
function newsposter($info,$sep = "",$class = "") {
  ...
  $res .= $locale['global_071'].showdate("longdate", $info['news_date']);
	...
	$res .= "</div>";
  return "<div class='newsauthor'><!--news_poster-->".$res;
}
...
function newsopts($info, $sep, $class = "") {
  ...
  if ($info['news_ext'] == "y" || $info['news_allow_comments']) $res .= "<div style='float:right;' ".$link_class.">".$info['news_reads'].$locale['global_074']."</div>\n";
	$res .= $sep." <a href='print.php?type=N&amp;item_id=".$info['news_id']."'".$link_class.">".$locale['global_075']."</a>\n";  
...
function articleposter($info, $sep = "", $class = "") {
	...
	$res = "";
	$link_class = $class ? " class='$class' " : "";
	$res = $locale['global_070']."<a href='profile.php?lookup=".$info['user_id']."'".$link_class.">".$info['user_name']."</a>\n";
	$res .= $locale['global_071'].showdate("longdate", $info['article_date']);
	$res .= $info['article_allow_comments'] ? $sep."\n" : "\n";
	$res .= "</div>";
	return "<div class='newsauthor'><!--article_poster-->".$res;
}

function articleopts($info, $sep = "", $class = "") {
	global $locale;
	$res = "";
	$link_class = $class ? " class='$class' " : "";
	if ($info['article_allow_comments']) { $res = "<a href='articles.php?article_id=".$info['article_id']."#comments'".$link_class.">".$info['article_comments'].($info['article_comments'] == 1 ? $locale['global_073b'] : $locale['global_073'])."</a> ".$sep."\n"; }
	$res .= "<div style='float:right;' ".$link_class.">".$info['article_reads'].$locale['global_074']." </div>\n";
	$res .= "<a href='print.php?type=A&amp;item_id=".$info['article_id']."'".$link_class.">".$locale['global_075']."</a>\n";
...
function itemoptions($item_type, $item_id) {
	...
	if ($item_type == "N") {
		if (iADMIN && checkrights($item_type)) { $res .= "<!--article_news_opts--> <a class='newsbutton' href='".ADMIN."news.php".$aidlink."&amp;action=edit&amp;news_id=".$item_id."'>".$locale['global_076']."</a>\n"; }
	} elseif ($item_type == "A") {
	if (iADMIN && checkrights($item_type)) { $res .= "<!--article_admin_opts--> <a class='newsbutton' href='".ADMIN."articles.php".$aidlink."&amp;action=edit&amp;article_id=".$item_id."'>".$locale['global_076']."</a>\n"; }
	}
	return $res;
}

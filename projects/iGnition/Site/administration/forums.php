...
if (isset($_GET['action']) && $_GET['action'] == "refresh") {
	...
	while ($data = dbarray($result)) {
		...
		$result2 = dbquery("SELECT * FROM ".DB_FORUMS." WHERE forum_parent='0' AND forum_cat='".$data['forum_id']."' ORDER BY forum_order");
		$k = 1;
...
function refresh($parent){
	$result2 = dbquery("SELECT * FROM ".DB_FORUMS." WHERE forum_parent='$parent' ORDER BY forum_order");
	$k = 1;
	$list = "";
	while ($data2 = dbarray($result2)) {
		$result3 = dbquery("UPDATE ".DB_FORUMS." SET forum_order='$k' WHERE forum_id='".$data2['forum_id']."'");
...
if (isset($_POST['save_cat'])) {
	...
	if (isset($_GET['t']) && $_GET['t'] == "cat") {
		...
		$data = dbarray(dbquery("SELECT * FROM ".DB_FORUMS." WHERE forum_cat='".$_GET['cat']."' AND forum_order='".$_GET['order']."'$parent"));
		$result = dbquery("UPDATE ".DB_FORUMS." SET forum_order=forum_order+1 WHERE forum_id='".$data['forum_id']."'");
		$result = dbquery("UPDATE ".DB_FORUMS." SET forum_order=forum_order-1 WHERE forum_id='".$_GET['forum_id']."'");
	}
	redirect(FUSION_SELF.$aidlink);
...
opentable($locale['550']);
	...
	if (dbrows($result) != 0) {
		...
		while ($data = dbarray($result)) {
			...
			echo "</tr>\n";
			$result2 = dbquery("SELECT * FROM ".DB_FORUMS." WHERE forum_cat='".$data['forum_id']."' AND forum_parent='0' ORDER BY forum_order");
			if (dbrows($result2)) {
				$k = 1;
				while ($data2 = dbarray($result2)) {
...
function forum_parent($data2, $result2, $k){
	...
	$forumR .= ($data2['forum_parent'] == 0 ) ? $data2['forum_name'] : "<img src='".IMAGES."subforum.png' alt='' style='vertical-align: middle;' />".$data2['forum_name'];
	$forumR .= "</span>\n";

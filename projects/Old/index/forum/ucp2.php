...
switch ($mode)
{
	case 'activate':
		...
		redirect(append_sid("http://www.winchelldesign.com/index.$phpEx"));
	break;
	...
	case 'logout':
		...
		meta_refresh(3, append_sid("http://www.winchelldesign.com/index.$phpEx"));
	
		$message = $message . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("http://www.winchelldesign.com/index.$phpEx") . '">', '</a> ');
	...
	case 'privacy':
		...
		$template->assign_vars(array(
			...
			'U_BACK'				=> append_sid("{$phpbb_root_path}ucp2.$phpEx", 'mode=login'),
	...
	case 'delete_cookies':
		...
		if (confirm_box(true))
		{
			...
			$message = $user->lang['COOKIES_DELETED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("http://www.winchelldesign.com/index.$phpEx") . '">', '</a>');
			trigger_error($message);
		}
		else
		{
	...
	case 'switch_perm':
		...
		$message = sprintf($user->lang['PERMISSIONS_TRANSFERRED'], $user_row['username']) . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("http://www.winchelldesign.com/index.$phpEx") . '">', '</a>');
		trigger_error($message);
	...
	case 'restore_perm':
		...
		$message = $user->lang['PERMISSIONS_RESTORED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("http://www.winchelldesign.com/index.$phpEx") . '">', '</a>');
		trigger_error($message);

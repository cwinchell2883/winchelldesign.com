...
if ($submit || $preview || $refresh)
{
  ...
  //Support Ticket MOD Beginn
	if (in_array($forum_id, $stforums) && ($mode == 'post'))
	{
		...
		$phpbbknowledge = request_var('phpbbknowledge', 0);
		$boardlink1 = utf8_normalize_nfc(request_var('boardlink', '', true ));
		$phpversion = utf8_normalize_nfc(request_var('phpversion', '', true ));
		$mysqlversion = utf8_normalize_nfc(request_var('mysqlversion', '', true ));
		$errselfsolution = utf8_normalize_nfc(request_var('errselfsolution', '', true ));
		$message		= utf8_normalize_nfc(request_var('message', '', true));
		$anti_support_message = utf8_normalize_nfc(request_var('message', '', true));

		$messagetemp = '';

		// phpBB Type Options field convert
		if ($phpbbtype == '1')
		{
		...
		// Message format
		$u1 = $user->lang['CST_PHPBBVERSION'];
		$u2 = $user->lang['CST_PHPBBTYPE'];
		$u3 = $user->lang['CST_MODS_SHORT'];
		$u4 = $user->lang['CST_KNOWLEDGE'];
		$u5 = $user->lang['CST_BOARDLINK'];
		$u6 = $user->lang['CST_PHPVER'];
		$u7 = $user->lang['CST_SQLVER'];
		$u8 = $user->lang['CST_BEFOREERR'];
		$u9 = $user->lang['CST_SELFSOLUTION'];
		$u0 = $user->lang['CST_HEAD_MSG'];
		...
		$messagetemp .= ($mysqlversion) ? "[b]" . $u7 . "[/b] " . $mysqlversion . "\n" : '';
		$messagetemp .= ($mysqlversion || $phpversion) ? "\n" : '';
		$messagetemp .= "[b]" . $u8 . "[/b]\n[i]" . $phpbbdescvf . "[/i]\n\n";
		$messagetemp .= "[b]" . $u9 . "[/b]\n[i]" . $errselfsolution . "[/i]\n\n";
		$messagetemp .= "[b]" . $u0 . "[/b]\n[i]" . $message . "[/i]";
...
$template->assign_vars(array(
	...
	//Begin Support Ticket System MOD
	# Replace Line
  ...
  //Beginn Support Ticket System MOD
	'CST_PHPBBVERSION'	=> ($preview || $submit) ? request_var('phpbbversion', '', true) : '2.0.',
	'CST_PHPBBTYPE'		=> request_var('phpbbtype', 0),
	'CST_MODS'			=> request_var('phpbbmods', 0),
	'CST_KNOWLEDGE'		=> request_var('phpbbknowledge', 0),
	'CST_BEFOREERR'		=> request_var('phpbbdescvf', '', true),
	'CST_SELFSOLUTION'	=> request_var('errselfsolution', '', true),
	'CST_BOARDLINK'		=> ($preview || $submit) ? request_var('boardlink', '') : 'http://',
	'CST_SQLVER'		=> request_var('mysqlversion', ''),
	'CST_PHPVER'		=> request_var('phpversion', ''),
	//End Support Ticket System MOD

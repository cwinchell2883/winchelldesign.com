...
####
	...
	if ($access['Group_ID'] >= 2) { header("Location: login.php"); die; }
?>
...
<div id="wrapper">
	<div id="content">
        ...
        <div id="main">        
        	...
        	<h1>Admin Settings - Manage Users</h1>
<?php
...
while ($row = $db->sql_fetchrow($users)) 
{
	// Usernames to Exclude from the listing (ie. Root Administrator)
	if($row['Username'] == "admin")
	{
		echo "<tr><td>";
		echo "<input type=\"hidden\" size=\"12\" name=\"username".$row['User_ID']."\" value=\"".$row['Username']."\">";
		echo "<input type=\"hidden\" size=\"12\" name=\"email".$row['User_ID']."\" value=\"".$row['Email']."\">";
		echo "<input type=\"hidden\" size=\"12\" name=\"group_id".$row['User_ID']."\" value=\"".$row2['Group_ID']."\">";
		echo "</td></tr>";
	}
	else
	{
		...
		while ($row2 = $db->sql_fetchrow($groups))
		{
			...
			// Exclude Root Administrator Group

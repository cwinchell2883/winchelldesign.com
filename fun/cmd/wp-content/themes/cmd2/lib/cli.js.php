...
var sessionId 			= <?php echo(isset($_SESSION['sessions_work'])?'false':"'_".md5(rand(1,5000000))."'"); ?>;
...
var scrollStep 			= <?php echo get_option('cli_scroll_step') ?>;
...
<?php if (get_option('cli_cursor_style') == 'block') { ?>
	if (cursorState == 1){
...
function initializeCLI(){
	...
	<?php if (get_option('cli_cursor_blink_time') == 0){
		if (get_option('cli_cursor_style') == 'block'){
	?>
	cursorElement.style.color = '<?php echo BG_COLOUR ?>';
	cursorElement.style.backgroundColor = '<?php echo FG_COLOUR ?>';
	<?php	}else{ ?>
	...
	if (!cursorBlinkThreadId){
		cursorBlinkThreadId = setInterval(cursorBlink,<?php echo get_option('cli_cursor_blink_time'); ?>);
	}
	<?php } ?>
...
function executeCommand(getCommand, postData){
	...
	var url = interpreter + "?" + prepareURI(getCommand) + "&requestId=" + requestId <?php 
		if (!isset($_SESSION['sessions_work'])){ /* no kidding. php5.2 appears to have pooched sessions */
			echo('+"&CSID="+sessionId');
		}else if (SID){
...
function processInputBuffer(input){
	...
	if (clientSideCommandsEnabled){
		...
		<?php if (get_option('cli_gui_url')) { ?>
				document.location.href = "<?php echo get_option('cli_gui_url'); ?>";
		<?php } else { ?>
				alert('No GUI link configured!');
		<?php } ?>
				return false;
			}
            END DISABLED COMMANDS */
...
function handleKeyEvent(e){
	var rval=false;
	var left, right, character = false, keyName=false, keyCode=false;
	if (waitingForServer){ 
		return rval; 
	...
	if(navigator.appVersion.indexOf('AppleWebKit') > 0){
		...
		if(e.type=='keyup' && keycodes[keyCode] && keycodes[keyCode].length == 1){
	...
	if (! (stickyState.CTRL || stickyState.ALT)){	
		if ((character.length == 1) && e &&	(e.keyCode != 13)){ // if it's a regular key
			if (eatIt){
				character = '';
				eatIt = false;
			}
	...
	if (multilineMode){
		...
		if (keyName == 'UP'){
			...
			if (previousNewline < 0){
				previousNewline = 0; previousPreviousNewline = 0;
			}
		...
		if (keyName == 'DOWN'){
			...
			if (nextNewline < 0){
				nextNewline = inputBuffer.length; nextNextNewline = nextNewline;
			}
...
//document.onkeydown=function(event){handleKeyEvent(event)}; 
//document.onkeyup=function(event){handleKeyEvent(event)};

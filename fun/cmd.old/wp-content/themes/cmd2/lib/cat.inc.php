	...
	//	
		...
		if(!$post || 
			...
			if(
				...
				if(GUI_URL){
					$u=
					e('<a href="'.get_option('cli_gui_url').'/?'.($status=='static'?'page_id':'p').'='.$id.'">Try the GUI</a>.');
				}
				e('</p>');
			}else{
				e('<div>');
...
function monochromify_and_rasterize_images_in($content){
	$fg_colour=array(
		'red'		=>get_option('cli_fg_colour_red'),
		'green'	=>get_option('cli_fg_colour_green'),
		'blue'	=>get_option('cli_fg_colour_blue')
	);
	
	$bg_colour=array(
		'red'		=>get_option('cli_bg_colour_red'),
		'green'	=>get_option('cli_bg_colour_green'),
		'blue'	=>get_option('cli_bg_colour_blue')
	);
	$lighter=$fg_colour; 
	$darker=$bg_colour;
